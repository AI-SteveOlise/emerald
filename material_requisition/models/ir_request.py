...
class IRRequest(models.Model):
    ...
    hod = fields.Many2one(
        comodel_name="hr.employee", related="requester.parent_id", string="H.O.D"
    )
    ...
    approve_request_ids = fields.One2many(
        comodel_name="ir.request.approve",
    ...
    account_id = fields.Many2one(
        "account.account",
        "Account",
        help="This is an expense account which will be Debited",
    )
    can_confirm = fields.Boolean(
        string="Ready to confirm",
        compute="_compute_confirm",
        help="Warehouse officer can confirm",
    )

    def _compute_confirm(self):
        self.can_confirm = (
            bool(self.approve_request_ids) and self._confirm_lines_valid()
        )
    ...
    def create(self, vals):
        default_ir_expense_account = (
            self.sudo().env.user.company_id.default_ir_expense_account
        )
        default_ir_journal_id = (
            self.sudo().env.user.company_id.default_ir_journal_id
        )
        account_vals = {
    ...
    def do_transfer(self):
        if self:
            ...
            print("88888888888888888888888888", stock_picking_id)
    ...
    def process(self, picking_id):
        ...
        immediate_transfer_wiz = self.env["stock.immediate.transfer"].search(
            [("pick_ids", "in", picking_id.ids)]
        )
        immediate_transfer_wiz and immediate_transfer_wiz.process()
        # hook up journal entries in here
        url = self.request_link()
        recipient = self.recipient("department_manager", self.department_id)
...
class IRRequestApprove(models.Model):
    ...
    transferred = fields.Boolean(string="Transferred", default=False, copy=False)
